...
def cargaDC4_20140207(item):
    ...
    patrimonio = Patrimonio(ns=item['ns'], apelido=item['apelido'], descricao=item['descricao'], \
                         tipo=tipoPatr, patrimonio=patrRack, equipamento=equipamento, \
                         modelo=equipamento.modelo, tamanho=equipamento.tamanho, \
                         agilis=False)
    
    patrimonio.save()
    
    historicoPai = HistoricoLocal(posicao=item['posicao'], \
                                  descricao='Verificado novo equipamento em visita ao NAP.', \
                                  data= datetime.date(2014,01,29), \
                                  patrimonio=patrimonio, endereco= endDet, estado=est)
