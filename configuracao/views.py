...
def has_help_text(request, app_name, model):
    return HttpResponse(ClassesExtra.objects.filter(content_type__app_label=app_name, content_type__model=model).count())

def tooltip(request, app_name, model):
    field_name = request.GET.get('field_name')
    try:
        fh = FieldsHelp.objects.get(model__content_type__app_label=app_name, model__content_type__model=model, field=field_name)
        ret = fh.help
