...
def extrato(request, pdf=False):
	...
	if request.GET.get('ano'):
	    ...
	    if pdf:
	        return render_to_pdf('financeiro/contacorrente.pdf', {'ano':ano, 'extrato':retorno}, filename='extrato_cc_%s.pdf' % ano)
            else:
                return render_to_response('financeiro/contacorrente.html', {'ano':ano, 'extrato':retorno}, context_instance=RequestContext(request))
	else:
	    anos = range(1990,datetime.datetime.now().year+1)
	    anos.sort(reverse=True)
	    return render_to_response('financeiro/pagamentos_mes.html', {'anos':anos}, context_instance=RequestContext(request))	    
...
def extrato_mes(request, pdf=False):
	...
	if request.GET.get('ano'):
	    ...
	    retorno = []
	    sa = True
	    for e in ExtratoCC.objects.filter(data_oper__year=ano, data_oper__month=mes).order_by('data_oper'):
		if sa:
		    sa = False
...
def financeiro_parciais(request, pdf=False):
	...
	if request.GET.get('termo'):
	    ...
	    termo = get_object_or_404(Termo, id=termo_id)
	    retorno = []
	    ps = ExtratoFinanceiro.objects.filter(termo=termo).values_list('parcial', flat=True)
	    parciais = []
	    for p in ps:
	        if p not in parciais:
		    parciais.append(p)
            parciais.sort()
		...
		tdia = Decimal('0.0')
		exi = {}
		for ef in ExtratoFinanceiro.objects.filter(termo=termo, parcial=parcial).order_by('data_libera', '-historico'):
